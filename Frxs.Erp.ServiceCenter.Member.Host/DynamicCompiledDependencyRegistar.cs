...
namespace Frxs.Erp.ServiceCenter.Member.Host
{
    ...
    public class DynamicCompiledDependencyRegistar : IDynamicCompiledDependencyRegistar
    {
        ...
        public void Register()
        {
            ...
            //            LogContent = "计时开始",
            //            LogSource = "Frxs.Erp.ServiceCenter.Member.Host.DynamicCompiledDependencyRegistar.Register",
            //            LogOperation = "注册初始化数据开始",
            //            LogIp = "localhost"
            //        }
            //    );
            //1.注册全局配置信息
            SystemOptionsManager.Current = new SystemOptions()
            {
                ...
                ActionDocResourcePaths = new string[] {
                    //System.Web.HttpContext.Current.Server.MapPath("~/Config/ActionsDocResource.config")
                },
                //自动输出SDK命名空间
                SdkNamespace = "Frxs.Erp.ServiceCenter.Member.SDK",
                //检测用户ID和用户名称是否提交(交给外部来配置怎么检测用户名称和用户ID是否提交了)
