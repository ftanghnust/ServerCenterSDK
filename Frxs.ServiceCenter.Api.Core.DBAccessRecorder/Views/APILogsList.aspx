...
<%   
    //上送参数
    var requestDto = this.RequestContext.RequestDto as Frxs.ServiceCenter.Api.Core.AccessRecorder.ApiLogsAction.ApiLogsActionRequestDto;

    //输出的数据
    var responseDto = this.ActionResult.Data as Frxs.ServiceCenter.Data.Core.PagedList<Frxs.ServiceCenter.Api.Core.AccessRecorder.Domain.AccessRecorder>;
...
<body style="font-family: Consolas; padding: 0px; margin: 0px">
        ...
        <tr>
            ...
            <td style="text-align: center;">
                <%if (responseDto.HasPreviousPage)
                  { %>
                <a style="color: #ffffff;" href="/Api?ActionName=API.Logs.List&format=view&data=<%:new {requestDto.PageSize,PageIndex= requestDto.PageIndex-1,requestDto.ApiName,requestDto.Ip,responseDto.TotalCount,requestDto.UsedTime }.SerializeObjectToJosn().UrlEncode()%>">Prev</a>
                <%} %>
                <%for (int i = requestDto.PageIndex > 10 ? requestDto.PageIndex - 9 : 0; i < requestDto.PageIndex + 10 && i < responseDto.TotalPages; i++)
                  {%>
                <a style="color: #ffffff;" href="/Api?ActionName=API.Logs.List&format=view&data=<%:new { requestDto.PageSize, PageIndex = i+1, requestDto.ApiName,requestDto.Ip,responseDto.TotalCount,requestDto.UsedTime }.SerializeObjectToJosn().UrlEncode()%>"><%:i+1 %></a>
                <%}%>
                <%if (responseDto.HasNextPage)
                  { %>
                <a style="color: #ffffff;" href="/Api?ActionName=API.Logs.List&format=view&data=<%:new {requestDto.PageSize,PageIndex= requestDto.PageIndex+1,requestDto.ApiName,requestDto.Ip,responseDto.TotalCount,requestDto.UsedTime }.SerializeObjectToJosn().UrlEncode()%>">Next</a>
                <%} %>
            </td>
    ...
    <table class="dataTable" style="width: 100%; margin: 0px; padding: 0px; border-collapse: collapse;">
        ...
        <%foreach (var item in responseDto)
          { %>
        <tr class="t">
            <td class="l id"><a onclick="" target="_blank" href="/Api?ActionName=API.Logs.Get&format=view&data=<%:new {item.Id}.SerializeObjectToJosn().UrlEncode()%>"><%:item.Id %></a></td>
            <td class="l an">
                <a target="_blank" href="/Api?ActionName=API.Logs.List&format=view&data=<%:new {item.ApiName,item.Id }.SerializeObjectToJosn().UrlEncode()%>"><%:item.ApiName %></a>
            </td>
            <td class="l cd">
                <%:item.Created %>
            </td>
