/*********************************************************
 * FRXS(ISC) zhangliang@frxs.com 2015/11/18 8:59:14
 * *******************************************************/
using System;
...
namespace Frxs.ServiceCenter.Api.Core.Actions
{
    ...
    /// </summary>
    [ActionName("API.Help"), ActionResultCache(15), DisablePackageSdk, AllowAnonymous, EnableRecordApiLog(false), Serializable, DisableDataSignatureTransmission]
    internal class HelpAction : ActionBase<HelpAction.HelpActionRequestDto, object>
    {
        ...
        public HelpAction(IActionSelector actionSelector, ITypeFinder typeFinder)
            : this()
        {
                  ...
                  Data = new
                  {
                      ...
                      Actions = from item in actionTypes
                                select new
                                {
                                    ...
                                    //接口描述
                                    Description = this.ActionDocResourceManager.GetDescription(item.ActionType.FullName),
                                    //请求对应的数据包
                                    RequestDTO = new
                                    {
                                        //请求DTO类型
                                        ...
                                        Propertys = item.ActionType.BaseType.GetGenericArguments()[0].GetProperties().Select(o => new
                                        {
                                            ...
                                            //描述信息
                                            Description = this.ActionDocResourceManager.GetDescription(string.Format("{0}.{1}", item.ActionType.BaseType.GetGenericArguments()[0].FullName, o.Name))
