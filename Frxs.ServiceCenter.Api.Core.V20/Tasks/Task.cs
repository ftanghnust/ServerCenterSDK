/*********************************************************
 * FRXS(ISC) zhangliang4629@163.com 6/8/2016 10:35:35 AM
 * *******************************************************/
using System;
using Autofac;
            ...
            try
            {
                ...
                if (this.RunOnOneWebFarmInstance)
                {
                    ...
                    if (taskSchedulerLocker.LeasedUntil.Value < DateTime.Now)
                    {
                        ...
                        //也就是说，在分布式的情况下，一个作业任务最大出现问题的机会只会有10分钟
                        //这样防止作业任务一致不起作用
                ...
                if (!instance.IsNull())
                {
                    instance.Execute(new TaskExecuteContext() { TaskScheduler = this._taskScheduler, TaskThread = taskThread });
                }
