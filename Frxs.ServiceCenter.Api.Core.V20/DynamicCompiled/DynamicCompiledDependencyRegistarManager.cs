/*********************************************************
 * FRXS(ISC) zhangliang4629@163.com 2015/12/29 12:36:24
 * *******************************************************/
using System;
...
namespace Frxs.ServiceCenter.Api.Core
{
    ...
    internal class DynamicCompiledDependencyRegistarManager
    {
        ...
        public static void Registar(string sourceFilePath)
        {
            ...
            {
                throw new ApiException("文件：{0} 不存在，请检查显示目录是否存在此文件，此文件为接口配置文件".With(sourceFilePath));
            }
            ...
            //默认加载系统dll文件
            string[] systemDlls = new string[] { "mscorlib.dll",
                                                 "System.dll",
                                                 "System.Core.dll",
                                                 "System.Web.dll",
                                                 "System.Linq.dll",
            ...
            Directory.GetFiles(binDirectoryPath, "*.dll", SearchOption.TopDirectoryOnly)
                .Select(Path.GetFileName).ToList().ForEach(assembly =>
                {
            ...
            if (compilerResults.Errors.HasErrors)
            {
                ...
                throw new ApiException("编译源文件：{0} 错误；错误详情：\r\n{1}"
            ...
            compilerResults.CompiledAssembly.GetTypes()
                .Where(type => type.IsPublic && !type.IsAbstract && typeof(IDynamicCompiledDependencyRegistar).IsAssignableFrom(type))
                .ToList().ForEach(type =>
            {
                //批量注册
                ((IDynamicCompiledDependencyRegistar)Activator.CreateInstance(type)).Register();
            });
        }
    }
}
