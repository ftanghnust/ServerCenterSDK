...
namespace Frxs.ServiceCenter.DataSync.Message.ApiHost.Promotion.Data
{
    /// <summary>
    ...
    public class DependencyRegistar : IDependencyRegistar
    {
        ...
        public void Register(ContainerBuilder containerBuilder, ITypeFinder typeFinder, SystemOptions systemOptions)
        {
            ...
            containerBuilder.Register<IDbContext>(c => new PromotionContext(() =>
            {
                ...
                //找出当前仓库数据存储介质服务器
                var dbConfig = sysDbConfigs.FirstOrDefault(o => o.CON_KEY.Equals("PROMOTION_{0}".With(currentWID), StringComparison.OrdinalIgnoreCase));
                if (dbConfig.IsNull())
                {
