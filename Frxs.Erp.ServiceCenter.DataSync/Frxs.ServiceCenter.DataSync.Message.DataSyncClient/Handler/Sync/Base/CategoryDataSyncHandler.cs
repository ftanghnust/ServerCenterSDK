...
namespace Frxs.ServiceCenter.DataSync.Message.DataSyncClient.Handler.Sync
{
    ...
    /// 数据库同步处理
    /// </summary>
    public class CategoryDataSyncHandler : DataSyncHandler
    {
        /// <summary>
        /// 
        /// </summary>
        public override void Execute()
        {
            ...
            //请求参数
            var request = new DataSyncBaseCategoryGetListRequest()
            {
                PageIndex = 0,
            ...
            //删除所有数据
            DbContextFactory.CreateBaseDataContext().Delete<Models.Category>();
            ...
            for (int page = 0; page < resp.Data.TotalPages; page++)
            {
                ...
                foreach (var item in resp.Data.Items)
                {
                    ...
                    dbContext.Categories.Add(item.MapTo<Models.Category>());
