/*********************************************************
 * FRXS(ISC) zhangliang4629@163.com 7/22/2016 10:45:31 AM
 * *******************************************************/
...
namespace Frxs.ServiceCenter.DataSync.Message.DataSyncClient.Handler.Sync
{
    ...
    /// 数据库同步处理
    /// </summary>
    public class ProductsAttributesPictureDataSyncHandler : DataSyncHandler
    {
        /// <summary>
        /// 
        /// </summary>
        public override void Execute()
        {
            ...
            //请求参数
            var request = new DataSyncBaseProductsAttributesPictureGetListRequest()
            {
                PageIndex = 0,
            ...
            //删除所有数据
            DbContextFactory.CreateBaseDataContext().Delete<Models.ProductsAttributesPicture>();
            ...
            for (int page = 0; page < resp.Data.TotalPages; page++)
            {
                ...
                foreach (var item in resp.Data.Items)
                {
                    ...
                    dbContext.ProductsAttributesPictures.Add(item.MapTo<Models.ProductsAttributesPicture>());
