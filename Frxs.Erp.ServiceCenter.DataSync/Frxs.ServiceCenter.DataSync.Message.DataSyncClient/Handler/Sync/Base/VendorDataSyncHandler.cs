...
namespace Frxs.ServiceCenter.DataSync.Message.DataSyncClient.Handler.Sync
{
    ...
    /// 数据库同步处理
    /// </summary>
    public class VendorDataSyncHandler : DataSyncHandler
    {
        /// <summary>
        /// 
        /// </summary>
        public override void Execute()
        {
            ...
            //请求参数
            var request = new DataSyncBaseVendorGetListRequest()
            {
                PageIndex = 0,
            ...
            //删除所有数据
            DbContextFactory.CreateBaseDataContext().Delete<Models.Vendor>();
            ...
            for (int page = 0; page < resp.Data.TotalPages; page++)
            {
                ...
                foreach (var item in resp.Data.Items)
                {
                    ...
                    dbContext.Vendors.Add(item.MapTo<Models.Vendor>());
