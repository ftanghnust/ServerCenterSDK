...
namespace Frxs.Erp.ServiceCenter.Product.MSSQLDAL
{
    ...
    /// 
    /// </summary>
    public class WProductsQueryDAO : BaseDAL, IWProductsQueryDAO
    {
        /// <summary>
        /// 读取SQL配置文件节点Name名称
        /// </summary>
        ...
        /// 查询参数
        /// </summary>
        public WProductsQuerySearchParams SearchParams { get; set; }
        ...
        {
            //构造SQL语句
            this.sql = "WITH LIST AS (" + this.GetSQLScriptByTable("WProductsQueryList");
            ...
            {
                sql += " AND pv.ProductId IN (SELECT ProductID FROM ProductsBarCodes WHERE BarCode=@BarCode)";
                this.sqlParametersBuilder.Add("BarCode", "%" + this.SearchParams.BarCode + "%");
            }

            if (this.SearchParams.VendorID.HasValue)
            {
                sql += " AND pv.VendorID=@VendorID";
                this.sqlParametersBuilder.Add("VendorID", this.SearchParams.VendorID.Value);
            }
        ...
        /// </summary>
        /// <returns></returns>
        public IList<WProductsQueryModel> GetList()
        {
            DBHelper helper = DBHelper.GetInstance();
            try
            {
                ...
                {
                    return this.ExecuteTolist<WProductsQueryModel>(dataReader);
                }
            }
            catch (Exception ex)
            {
