...
{
    public class WProductsBuyEmpListDAO : BaseDAL, IWProductsBuyEmpListDAO
    {
        /// <summary>
        /// 读取SQL配置文件节点Name名称
        /// </summary>
        protected override string TableName { get { return "WProductsBuyEmp"; } }

        /// <summary>
        /// 
        /// </summary>
        public WProductsBuyEmpListParams SearchParams { get; set; }
        ...
        {
            //构造SQL语句
            this.sql = " ( " + this.GetSQLScriptByTable("WProductsBuyEmpList");
            ...
            if (this.SearchParams.HasBuyEmp.HasValue)//是否设置采购员
            {
                if (this.SearchParams.HasBuyEmp == 1)//已设置
                {
                    sql += " AND wp.WProductID IN (SELECT DISTINCT WProductID FROM WProductsBuyEmp)";
                }
                else if (this.SearchParams.HasBuyEmp == 0)//未设置
                {
                    sql += " AND wp.WProductID NOT IN (SELECT DISTINCT WProductID FROM WProductsBuyEmp)";
                }
            }

            if (!string.IsNullOrWhiteSpace(this.SearchParams.BuyEmpName))
            {
                sql += " AND wp.WProductID IN (SELECT t1.WProductID FROM WProductsBuyEmp t1,WarehouseEmp t2 WHERE t1.EmpID=t2.EmpID AND t2.EmpName LIKE @BuyEmpName)";
                this.sqlParametersBuilder.Add("BuyEmpName", "%" + this.SearchParams.BuyEmpName + "%");
            }
            ...
            catch (Exception ex)
            {
                ...
                (
                    new NormalLog
                    {
                        ...
                        LogOperation = "Frxs.ServiceCenter.Product.MSSQLDAL.WProductsBuyEmpListDAO.GetCount",
        ...
        /// </summary>
        /// <returns></returns>
        public IList<WProductsBuyEmpListModel> GetList()
        {
            DBHelper helper = DBHelper.GetInstance();
            try
            {
                ...
                {
                    return this.ExecuteTolist<WProductsBuyEmpListModel>(dataReader);
                }
            }
            catch (Exception ex)
            {
                ...
                (
                    ...
                    {
                        LogSource = helper.DataSource,
                        LogOperation = "Frxs.ServiceCenter.Product.MSSQLDAL.WProductsBuyEmpListDAO.GetList",
                        LogContent = exceptionSql,
                        LogTime = DateTime.Now
                    },
                    ex
