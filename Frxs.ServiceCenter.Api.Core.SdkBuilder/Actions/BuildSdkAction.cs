...
/*********************************************************
 * FRXS(ISC) ftanghnust@gmail.com 2016/2/18 9:19:42
 * *******************************************************/
using System;
...
namespace Frxs.ServiceCenter.Api.Core.SdkBuilder.Actions
{
    /// <summary>
    /// 生成客户端SDK类文件
    /// </summary>
    [ActionName("API.BuildSdk"), Serializable, DisablePackageSdk, AllowAnonymous, EnableRecordApiLog(false), DisableDataSignatureTransmission]
    public class BuildSdkAction : ActionBase<BuildSdkAction.BuildSdkActionRequestDto, BuildSdkAction.BuildSdkActionResponseDto>
    {
        ...
        /// 跳过属性集合
        /// </summary>
        private readonly string[] _skipPropertys = new string[] { "UserId", "UserName" };
        ...
        public BuildSdkAction(IActionSelector actionSelector)
        {
                ...
                while ((theEntry = zipInputStream.GetNextEntry()) != null)
                {
                    ...
                    while (true)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                string source = streamReader.ReadToEnd().Replace("namespace Frxs.ServiceCenter.Api.Core.SDK", "namespace {0}".With(@namespace));
        ...
        private StringBuilder CreateUsingBlock(IActionDescriptor actionDescriptor)
        {
            ...
            codeBlockStringBuilder.Append(System.Environment.NewLine);
            codeBlockStringBuilder.Append("* FRXS(ISC) System Auto-Generation At {0}".With(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")));
            codeBlockStringBuilder.Append(System.Environment.NewLine);
            codeBlockStringBuilder.Append("* *********************************************************");
            codeBlockStringBuilder.Append(System.Environment.NewLine);
        ...
        private string CreateComplexClass(Type type, string otherName)
        {
            ...
            foreach (string item in this.ActionDocResourceManager.GetDescription(type.FullName).Split(Environment.NewLine.ToCharArray()))
            {
                sb.Append("/// " + item.Trim());
                ...
                type.GetProperties().ToList().ForEach(t =>
                 {
                     sb.Append("\t/// <summary>");
                     sb.Append(Environment.NewLine);
                     foreach (string item in this.ActionDocResourceManager.GetDescription(string.Format("{0}.{1}", type.FullName, t.Name)).Split(Environment.NewLine.ToCharArray()))
                     {
                         sb.Append("\t/// " + item.Trim());
                         sb.Append(Environment.NewLine);
                     }
            ...
            else
            {
                ...
                foreach (var item in Enum.GetValues(type))
                {
                    ...
                    enumStringBulder.Append(Environment.NewLine);
                    foreach (string x in this.ActionDocResourceManager.GetDescription(string.Format("{0}.{1}", type.FullName, item.ToString())).Split(Environment.NewLine.ToCharArray()))
                    {
        ...
        private string CreateMainClass(ActionDescriptor actionDescription, DtoTypeEnum dtoType)
        {
            ...
            sb.Append("/// <summary>");
            sb.Append(System.Environment.NewLine);
            foreach (string item in this.ActionDocResourceManager.GetDescription(actionDescription.ActionType.FullName).Split(Environment.NewLine.ToCharArray()))
            {
                sb.Append("/// " + item.Trim());
                sb.Append(Environment.NewLine);
            }
        ...
        /// </summary>
        /// <returns></returns>
        public override ActionResult<BuildSdkAction.BuildSdkActionResponseDto> Execute()
        {
            //命名空间
            string @sdkNamespace = SystemOptionsManager.Current.SdkNamespace;
            ...
            if (!string.IsNullOrEmpty(this.RequestDto.SaveType))
            {
                ...
                if (this.RequestDto.SaveType.ToLower() == "dll")
                {
                    ...
                    //输出dll保存地址
                    string dllSavePath = System.IO.Path.Combine(dllSaveDirectory, dllname);

                    //保存DLL注释文件XML路径
                    string docSavePath = System.IO.Path.Combine(dllSaveDirectory, "{0}.XML".With(@sdkNamespace));
                ...
                else if (this.RequestDto.SaveType.ToLower() == "source")
                {
                    ...
                    this.OutPutCSProj(request, response, sdkBasesources, tempPath);
        ...
        private void GetRequestSources(IList<KeyValuePair<string, string>> request, ActionDescriptor actionDescription)
        {
            ...
            request.Add(new KeyValuePair<string, string>("{0}Request.cs".With(actionName), CreateUsingBlock(actionDescription) + CreateNamespaceBlock(SystemOptionsManager.Current.SdkNamespace, DtoTypeEnum.Request).ToString() + Environment.NewLine +
                         "{" + Environment.NewLine +
                               requestStr +
                         "}"));
        }
        ...
        private void GetResponseSources(IList<KeyValuePair<string, string>> response, ActionDescriptor actionDescription)
        {
            ...
            if (respDtoType != typeof(NullResponseDto))       //下送参数为NullResponseDto不必输出
            {
                ...
                response.Add(new KeyValuePair<string, string>("{0}Resp.cs".With(actionName), CreateUsingBlock(actionDescription) + CreateNamespaceBlock(SystemOptionsManager.Current.SdkNamespace, DtoTypeEnum.Response).ToString() + Environment.NewLine +
                         "{" + Environment.NewLine +
                               respStr +
                         "}"));
            }
        ...
        /// <param name="path"></param>
        private void OutPutCSProj(IList<KeyValuePair<string, string>> request, IList<KeyValuePair<string, string>> response, IList<KeyValuePair<string, string>> sdkBasesources, string path)
        {
            string @namespace = SystemOptionsManager.Current.SdkNamespace;
